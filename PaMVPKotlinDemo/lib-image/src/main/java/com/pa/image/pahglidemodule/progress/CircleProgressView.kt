...
class CircleProgressView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : ProgressBar(context, attrs, defStyleAttr) {
    ...
    private var mReachBarSize = Utils.dp2px(getContext(), 2f) // 未完成进度条大小
    private var mNormalBarSize = Utils.dp2px(getContext(), 2f) // 未完成进度条大小
    private var mReachBarColor = Color.parseColor("#108ee9") // 已完成进度颜色
    private var mNormalBarColor = Color.parseColor("#FFD3D6DA") // 未完成进度颜色
    private var mTextSize = Utils.sp2px(getContext(), 14f) // 进度值字体大小
    ...
    private var mRadius = Utils.dp2px(getContext(), 20f) // 半径
    ...
    private var mInnerPadding = Utils.dp2px(getContext(), 1f) // 内部圆与外部圆间距
    ...
    private var mOuterSize = Utils.dp2px(getContext(), 1f) // 外层圆环宽度
    ...
    var reachBarSize: Int
        get() = mReachBarSize
        set(reachBarSize) {
            mReachBarSize = Utils.dp2px(context, reachBarSize.toFloat())
            invalidate()
        }
    ...
    var normalBarSize: Int
        get() = mNormalBarSize
        set(normalBarSize) {
            mNormalBarSize = Utils.dp2px(context, normalBarSize.toFloat())
            invalidate()
        }
    ...
    var textSize: Int
        get() = mTextSize
        set(textSize) {
            mTextSize = Utils.sp2px(context, textSize.toFloat())
            invalidate()
        }
    ...
    var radius: Int
        get() = mRadius
        set(radius) {
            mRadius = Utils.dp2px(context, radius.toFloat())
            invalidate()
        }
    ...
    var innerPadding: Int
        get() = mInnerPadding
        set(innerPadding) {
            mInnerPadding = Utils.dp2px(context, innerPadding.toFloat())
    ...
    var outerSize: Int
        get() = mOuterSize
        set(outerSize) {
            mOuterSize = Utils.dp2px(context, outerSize.toFloat())
            invalidate()
        }
